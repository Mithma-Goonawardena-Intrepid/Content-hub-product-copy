...
const assets = {
  "/index.mjs.map": {
    ...
    "etag": "\"745e3-myIPzIjZoGphe+fXfLWul8xSBDk\"",
    "mtime": "2026-07-10T05:10:41.666Z",
    "size": 476643,
    "path": "index.mjs.map"
  },
  "/index.mjs": {
    "type": "text/javascript; charset=utf-8",
    "etag": "\"1d6c6-ptENSaWOIjXXa0KqcQOCba+sdGo\"",
    "mtime": "2026-07-10T05:10:41.655Z",
    "size": 120518,
    "path": "index.mjs"
  }
};
...
const getEnvValue = (primaryKey, fallbackKey) => {
  ...
  const fallbackValue = normalizeEnvValue(
    fallbackKey ? process.env[fallbackKey] : void 0
  );
  if (fallbackValue) {
...
const PRODUCT_CONTENT_TYPE_UID = "product";
...
const normalizeManagementHost = (host) => {
  ...
  return hostnameOnly.replace(/^cdn\./, "api.").replace(/^eu-cdn\./, "eu-api.");
};
const getProductManagementHost = () => normalizeManagementHost(runtimeConfig.runtimeHost);
const getLocaleQueryParam = (localeIso) => {
...
const getErrorDetail = (payload, fallback) => {
  var _a;
  if (!payload) {
    return fallback;
  }
  ...
  return (_a = payload.error_message) != null ? _a : fallback;
};
const updateProduct = async (uid, fields, localeIso = "en") => {
  var _a;
  ...
  const payload = contentType.includes("application/json") ? await response.json() : void 0;
  if (!response.ok) {
...
const _uid__patch = defineEventHandler$2(async (event) => {
  var _a, _b;
  try {
    const uid = getRouterParam$1(event, "uid");
    if (!uid) {
      throw createError$2({ statusCode: 400, statusMessage: "Product uid is required" });
    }
    const requestUrl = new URL(((_b = (_a = event.node) == null ? void 0 : _a.req) == null ? void 0 : _b.url) || "/", "http://localhost");
    const locale = requestUrl.searchParams.get("locale") || "en";
    const fields = await readJsonBody(event).catch(() => ({}));
    await updateProduct(uid, fields, locale);
    return {};
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to update product";
    throw createError$2({ statusCode: 500, statusMessage: message });
  }
});

const _uid__patch$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: _uid__patch
}, Symbol.toStringTag, { value: 'Module' }));
...
const sortByMarketingRatingDesc = (products) => [...products].sort((a, b) => {
  var _a, _b;
  return ((_a = b.marketing_rating) != null ? _a : -Infinity) - ((_b = a.marketing_rating) != null ? _b : -Infinity);
});
...
const isActiveByEffectiveTo = (product) => {
...
const applyDeliveryLikeFilters = (products) => {
  return products.filter(
    (product) => product.published_to_web === true && isActiveByEffectiveTo(product)
  );
};
const buildProductsUrl = (localeIso, skip, limit) => {
...
const fetchProductsBatch = async (localeIso, skip, limit) => {
...
const extractProducts = (response) => {
  if (Array.isArray(response == null ? void 0 : response.items)) {
    return response.items;
  }
  if (Array.isArray(response == null ? void 0 : response.entries)) {
    return response.entries;
  }
  return [];
};
const fetchAllProducts = async (localeIso = "en", fetchFullList, _options) => {
...
const index_get = defineEventHandler$2(async (event) => {
  ...
  try {
    ...
    const fetchFullList = fetchFullListParam === void 0 || fetchFullListParam === null || fetchFullListParam === "true" || fetchFullListParam === "1";
